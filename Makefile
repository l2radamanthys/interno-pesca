DB_NOMBRE_DEL_DUMP= ~/Dropbox/Trabajo/villa-cristina/backups/villa_backup_`date +'%Y%m%d_%Hhs%Mmin'`.dump
DB_DUMP_MAS_RECIENTE=`ls -Art ~/Dropbox/Trabajo/villa-cristina/backups/villa_backup_*.dump  | tail -n 1`
NOMBRE_BD=villa_cristina
...
compandos:
	...
	@echo "	${G}version_fix${N}"
	@echo "	${G}version_release${N}"
	@echo ""
...
blacked:
	@pipenv run python -m black . --exclude=django-cli-helper

borrar_bd_pruebas:
	dropdb --if-exists test_tablero_ddc
